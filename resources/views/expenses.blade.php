			...
			<main class="flex-1 overflow-y-auto">
				<div
					...
					<div class="mb-8 flex items-center justify-between">
                        ...
                        <div class="inline-flex items-center justify-center gap-2 px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-medium rounded-lg hover:shadow-lg hover:shadow-emerald-500/50 transition-all duration-200 group" x-data>
                             <a href="{{ route('export.exportExpenses') }}">
                            Export as Excel
                             </a>
                        </div>
					</div>
                    ...
                    @else
                        @php
                            $selectedYear = $pickedYear ?? now()->year;
                            $selectedMonth = $pickedMonth ?? 0;
                        @endphp
                        {{-- Filter by date bar --}}
                        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 mb-6">
                            <!-- Header with title and optional reset -->
                            <div class="flex items-center justify-between mb-4">
                                <div class="flex items-center gap-2 text-slate-700">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewbox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-slate-500 w-5 h-5">
                                        <rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect>
                                        <line x1="16" y1="2" x2="16" y2="6"></line>
                                        <line x1="8" y1="2" x2="8" y2="6"></line>
                                        <line x1="3" y1="10" x2="21" y2="10"></line>
                                    </svg>
                                    <h3 class="text-lg font-semibold text-slate-900">Filter expenses</h3>
                                </div>
                                    <a href="{{ route('expenses.index') }}" class="px-3 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm rounded transition-colors">Reset</a>
                             
                            </div>

                            <form method="GET" action="{{ route('expenses.index') }}" class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
                                <div>
                                    <label for="filter_year" class="block text-xs font-medium text-slate-500 mb-1">Year</label>
                                    <select name="year" id="filter_year" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-slate-900 text-sm transition-all">
                                ...
                                <div>
                                    <label for="filter_month" class="block text-xs font-medium text-slate-500 mb-1">Month</label>
                                    <select name="month" id="filter_month" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-slate-900 text-sm transition-all">
                                        <option value="0">All months</option>
                                        @for ($m = 1; $m <= 12; $m++)
                                            @php $monthLabel = \Carbon\Carbon::createFromDate(null, $m, 1)->translatedFormat('F'); @endphp
                                            <option value="{{ $m }}" {{ (int) $selectedMonth === $m ? 'selected' : '' }}>{{ $monthLabel }}</option>
                                        @endfor
                                    </select>
                                </div>
                                <div class="flex justify-start sm:justify-end">
                                    <button type="submit" class="px-4 py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-medium rounded-lg hover:shadow-lg hover:shadow-emerald-500/50 transition-all duration-200 text-sm">
                                        Apply
                                    </button>
                                </div>
                            </form>
                        ...
                        @if ($colocation->expenses_count > 0)
                            ...
                            <div class="bg-white border border-slate-200 rounded-2xl shadow-sm hover:shadow-md transition-shadow duration-300 p-8">
                                <div
                                    ...
                                    @if($colocation->expenses->count() == 0) 
                                        <div class="text-center text-black font-semibold w-full text-xl">
                                            <p>There is no expenses in this date</p>
                                        </div>
                                    @endif
